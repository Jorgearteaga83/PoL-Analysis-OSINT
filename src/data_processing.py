...
def ratio_to_float(x: Any) -> float:
    """
    Converts an EXIF ratio object or tuple representation to a float.

    Args:
        x (Any): The ratio object or tuple/list of two numbers to convert.
...
def dms_to_decimal(dms: Any, ref: str) -> Optional[float]:
    """
    Converts Degrees, Minutes, Seconds (DMS) format to a decimal degree format.

    Args:
        dms (Any): The DMS coordinates as a list/tuple of ratios.
        ref (str): The reference direction (N, S, E, W).

    Returns:
        Optional[float]: The decimal degree value, or None if conversion fails.
    """
    try:
...
def extract_exif(path: Path) -> Tuple[bool, bool, Optional[float], Optional[float]]:
    ...
    Args:
        path (Path): The file path to the image.

    Returns:
        Tuple[bool, bool, Optional[float], Optional[float]]: A tuple containing (exif_readable, gps_present, lat, lon).
    """
    try:
        img = Image.open(path)
        exif = img.getexif()
...
def normalize_dataset(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Normalizes a raw social media dataset into a standardized format with unified column names.

    Args:
        df_raw (pd.DataFrame): The raw dataframe to normalize.

    Returns:
        pd.DataFrame: The normalized and cleaned dataframe.
    """
    df = df_raw.copy()
    out = pd.DataFrame()
