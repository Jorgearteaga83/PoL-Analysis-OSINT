...
def ensure_directories():
    """
    Ensures that necessary directories exist for data, output, and images.
...
class OSINTCleanGUI(tk.Tk):
    """
    Main application window for the OSINT Dataset Analysis tool.
    """
    ...
    def setup_style(self):
        """
        Configures the visual styling of the Tkinter application.
    ...
    def build_ui(self):
        """
        Constructs the user interface components of the main window.
    ...
    def poll_queue(self):
        """
        Periodically checks the task queue for messages from background threads.
    ...
    def run_in_background(self, target, *args):
        """
        Executes a given function in a background thread to prevent GUI freezing.
    ...
    def upload_dataset(self):
        """
        Opens a file dialog to select a dataset and starts the loading process in the background.
    ...
    def _async_load_dataset(self, path: str):
        """
        Loads and normalizes the dataset from the given file path asynchronously.
    ...
    def handle_dataset_loaded(self, data):
        """
        Updates the GUI once the dataset has been successfully loaded in the background.

        Args:
            data (tuple): A tuple containing the normalized DataFrame and the filename.
    ...
    def filtered_df(self) -> pd.DataFrame:
        """
        Filters the loaded dataset based on user-selected target account and time window.

        Args:
            None

        Returns:
            pd.DataFrame: A filtered and sorted pandas DataFrame.
        """
    ...
    def make_tree(self, columns, headings, widths):
        """
        Creates a highly configurable Treeview widget with scrollbars.

        Args:
            columns (list): List of column identifiers.
            headings (list): List of column display names.
            widths (list): List of column widths in pixels.

        Returns:
            ttk.Treeview: The constructed Treeview widget.
        """
    ...
    def embed_plot(self, fig):
        """
        Embeds a Matplotlib figure into the main content frame.

        Args:
            fig (matplotlib.figure.Figure): The Matplotlib figure to embed.

        Returns:
            FigureCanvasTkAgg: The canvas widget containing the plot.
        """
    ...
    def show_overview(self):
        """
        Displays a high-level overview of the filtered dataset, including counts and a preview table.
        ...
        def on_double_click(_event):
            """
            Handles double-click events on the Treeview to open the post URL.

            Args:
                _event (tk.Event): The triggered event.
            ...
            item = tree.selection()
            if item: 
                self.open_url(str(tree.item(item[0], "values")[-1]))

        tree.bind("<Double-1>", on_double_click)

    def show_temporal(self):
        """
        Displays temporal analysis charts (posts per day, hour, weekday, and a heatmap).
        ...
        def embed_plot_in_tab(fig, parent):
            """
            Embeds a Matplotlib figure into a specific notebook tab frame.

            Args:
                fig (matplotlib.figure.Figure): The Matplotlib figure to embed.
                parent (ttk.Frame): The parent frame to host the canvas.
    ...
    def show_sentiment(self):
        """
        Displays sentiment analysis of post captions along with a trend graph.
    ...
    def show_raw(self):
        """
        Displays the raw post data in a tabular format.
    ...
    def show_leakage(self):
        """
        Analyzes local images for EXIF metadata leakage, displaying them and their properties.
        ...
        right.rowconfigure(0, weight=1, uniform="group1")
        ...
        def on_select(_event):
            """
            Handles tree item selection to load the corresponding image and display its details.
        ...
        def on_double_click(_event):
            """
            Handles double-click events to open the original image URL in the browser.
    ...
    def generate_report(self):
        """
        Initiates the asynchronous generation of an HTML intelligence report for the target.
    ...
    def _async_generate_report(self, df_filtered: pd.DataFrame, target_user: str):
        """
        Asynchronously constructs and writes a comprehensive OSINT HTML report.

        Args:
            df_filtered (pd.DataFrame): The filtered dataset to analyze.
            target_user (str): The specific user account the report targets.
        ...
        try:
            ...
            if not location_counts.empty: 
                loc_analysis_html += location_counts.to_frame().to_html()
            else: 
            ...
            if not cat_data.empty:
                def get_unique_locations(s):
                    """
                    Extracts unique non-empty location strings from a series.

                    Args:
                        s (pd.Series): A series of location strings.

                    Returns:
                        str: A comma-separated string of unique locations.
                    """
            ...
            for _, row in mood_drops.iterrows(): 
                anomalies.append(f"<b>Mood Drop:</b> Significant negative sentiment post on {row['timestamp_utc']:%Y-%m-%d}.")
